# Instalação do Windows 11 em Máquina Virtual (VirtualBox)

## Objetivo

Criar um ambiente virtual para praticar instalação de sistema operacional e configuração inicial de um computador em laboratório de suporte de TI.

## Ferramentas utilizadas

- Oracle VirtualBox
- Windows 11 ISO
- Computador host (Windows)

## Criação da máquina virtual

1. Abrir o Oracle VirtualBox
2. Clicar em **Novo**
3. Definir o nome da máquina virtual
4. Selecionar o sistema operacional **Windows 11 (64-bit)**
5. Configurar memória RAM
6. Configurar número de CPUs
7. Criar um disco virtual
8. Selecionar a imagem ISO do Windows 11

## Instalação do Windows

1. Iniciar a máquina virtual
2. Selecionar idioma e layout de teclado
3. Escolher a edição do Windows
4. Aceitar os termos de licença
5. Selecionar o disco virtual criado
6. Iniciar a instalação do sistema

## Configuração inicial

Após a instalação foram realizadas as configurações iniciais:

- Seleção de região
- Configuração de teclado
- Criação de utilizador local
- Configuração básica do sistema

## Resultado

O Windows 11 foi instalado com sucesso dentro de uma máquina virtual utilizando o VirtualBox.

## Habilidades praticadas

- Virtualização
- Instalação de sistema operacional
- Configuração de máquina virtual
- Configuração inicial do Windows
- Documentação técnica
